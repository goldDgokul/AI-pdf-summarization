...
def main():
    ...
    st.markdown("Upload a PDF document and get intelligent summaries Running Locally with Ollama.")
    ...
    if 'summarizer' not in st.session_state:
        # This instantiation should now work because load_dotenv() was called
...
def process_pdf(uploaded_file, summary_type, max_tokens, show_analysis, show_quotes):
    ...
    with st.spinner(f"🤖 Generating {summary_type} summaries..."):
        ...
        try:
            # We updated summarizer.py to handle chunking, so this should work
    ...
    with st.expander("📈 Processing Statistics"):
        ...
        # Check if 'Compression Ratio' column exists before calculating mean
        if 'Compression Ratio' in summary_df.columns:
